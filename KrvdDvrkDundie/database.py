...
from KrvdDvrkDundie.settings import DATABASE_PATH, EMAIL_FROM
from KrvdDvrkDundie.utils.email import check_valid_email, send_email
...
class ORM:
    ...
    MAPPING = {
        "people": "KrvdDvrkDundie.models.Person",
        "balance": "KrvdDvrkDundie.models.Balance",
        "movement": "KrvdDvrkDundie.models.Movement",
        "users": "KrvdDvrkDundie.models.User",
    }
    ...
    def get_by_pk(self, pk):
        ...
        try:
            if hasattr(self[0], "pk"):
                return ResultList(item for item in self if item.pk == pk).first()
            return ResultList(item for item in self if item.person.pk == pk).first()
        except KeyError:
            raise NotFoundError(f"{pk} not found")
...
def connect() -> dict:
    """Connects ti the database, returns dice data"""
    try:
...
def add_person(db, instance):
    ...
    if created:
        ...
        send_email(EMAIL_FROM, instance.pk, "Your KrvdDvrkDundie password", password)
...
def set_initial_password(db, instance):
    ...
    user = User(person=instance)  # Password generate by model
    db[User].append(user)
...
def add_movement(db, person, value, actor="system"):
    ...
    db[Balance].append(Balance(person=person, value=sum([item.value for item in movements])))
